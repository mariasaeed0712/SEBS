...
DEM = readmap('analysis_run2/clone_files/DEM_90.map')	# Digital Elevation Model [m]
nd = readmap('analysis_run2/clone_files/NDVI_90.map') # NDVI map [-]
T = readmap('analysis_run2/clone_files/Temp_90.map') # Surface temperature [Kelvin]
albedo = readmap('analysis_run2/clone_files/albedo_90.map') # Albedo map [-]
ems = readmap('analysis_run2/clone_files/EMISSIVITY_90.map') # emissivity [-]

# parameters

Trans = 0.788606 #transmissivity [0-1]
Lat = 30.625 #Latitude [dd]
DOY = 43.0 #Day of year
Time = 10.62 #Time of overpass [decimal hours]
z_pbl = 1000.0#PBL height [m]
alt_ms = 2.0 #Measurement height [m]
u_s = 2.0 #Wind speed [m/s]
t_s = 17.10 #Air temperature [Celcius]
p_s = 99500.0 #Air pressure [Pa]
hr_s = 0.5055 #Relative humidity [0-1]
z_ms = alt_ms

# Define output files
lemap = 'analysis_run2/outputs/le.map'
hmap = 'analysis_run2/outputs/h.map'
gmap = 'analysis_run2/outputs/g0.map'
rnmap = 'analysis_run2/outputs/rn.map'
evapfrmap = 'analysis_run2/outputs/evapfr.map'
etmap = 'analysis_run2/outputs/et.map'
